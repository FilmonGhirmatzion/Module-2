...
select name , (gnp * 1000000 /population ) as per_capital_gnp
from country
...
SELECT continent, sum(surfacearea) as surface from country group by continent order by surface DESC;

-- 14. The highest population density (population divided by surface area) of all 
-- countries in the world. 
-- (highest population density in world: 26277.7777)
select max(population / surfacearea) as highes_population_density
from country;
...
select name, abs(gnp - gnpold) as absdiff, (gnp - gnpold) as diff
from country 
...
-- (45 rows)
select district,  count(*) as Cities_in_each_State
from city
where countrycode ='USA'
group by district
...
select countrycode,  count(*) as Cities_in_a_coutry
from city
group by countrycode
order by Cities_in_a_coutry desc;
...
select   countrycode, max(population)    as population_of_largest_city
from city
group by countrycode
